...
#
#
# svd_predict = matrix_factorization.svd_predict
# svd_train = matrix_factorization.svd_train
...
sA, _ = train_and_test_from_sparse(sA, ratio=0.008)

# sA = sA.tocsr()
# sA = sA[sA.getnnz(1) > 0][:, sA.getnnz(0) > 0]
# sA = sA.tocoo()

sA_train, sA_test = train_and_test_from_sparse(sA, ratio=0.9)
del sA

mu, bu, bi, pu, qi, rmse_arr, rmse_t_arr = svd_train(sA_train, sA_test, k_order=20, gamma=0.005, beta=0.02, num_of_iters=20)
